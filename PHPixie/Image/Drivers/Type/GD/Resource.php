 ...
 * GD image resource.
 */
require_once dirname(__DIR__)."/../Driver/Resource.php";
...
class Resource extends \PHPixie\Image\Drivers\Driver\Resource
{
    ...
    public function scale($scale,$pa=array()) {
    ...
    public function overlay($layer, $x = 0, $y = 0,$pct=0) {
        
        imagealphablending($this->image, true);
        $opacity=$pct;
        $w = imagesx($layer->image());
        $h = imagesy($layer->image());       
        $cut = imagecreatetruecolor($layer->width, $layer->height); 
        imagecopy($cut, $this->image, 0, 0, $x, $y, $layer->width, $layer->height);
        $opacity = 100 - $opacity;       
        imagecopy($cut, $layer->image(), 0, 0, 0 ,0, $layer->width, $layer->height);
        imagecopymerge($this->image, $cut, $x,$y, 0, 0, $layer->width, $layer->height, $opacity);
