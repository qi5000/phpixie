...
abstract class Resource
{
     ...
     */
    public function resize($width = null, $height = null, $fit = true,$f=false)
    {
        ...
        $pa=array();
        if($f){
            $pa=array('width'=>$width,'height'=>$height);
        }
        $this->scale($scale,$pa);
    ...
    public function text(
        ...
        if ($wrapWidth != null) {
            ...
            for ($i=0; $i <mb_strlen($text,$charset) ; $i++) { 
                ...
                $dimensions = imagettfbbox($size, 0, $fontFile, $temp); //
                $textWidth = abs($dimensions[4] - $dimensions[0]);  //计算文字宽度           
                if($textWidth-$wrapWidth>=0){
                    $result[]=$temp; //将当前文字作为一行存储 
                    $temp=''; //清除缓存
                }
                if ($i == mb_strlen($text,$charset) - 1){  //最后一行
                    $result[]=$temp;
                    $temp='';
                }
            }
